...
def calculate_nrmse(
    true, pred, time_vector: np.ndarray, dt_freq: str = "1D", min_obs: int = 10
):
    """
    Parameters:
    -----------
    ...
    if dt_freq is not None:
        ...
        rmse_pred = np.sqrt(np.square(da_diff))

    # mask score if num obs < min_
    if dt_freq is not None:
        vcount = da_diff.resample(time=dt_freq).count().values
    else:
        # TODO: check resampling if true
        vcount = da_diff.count().values
    ...
    # normalize the score
    nrmse_score = 1.0 - rmse_pred / rmse_true
    ...
    return RMSEStats(rmse_mean, rmse_std, nrmse_mean, nrmse_std)


def compute_ts_stats(
    data: np.ndarray, time_vector: np.ndarray, dt_freq: str = "1D", stat: str = "diff"
):
...
def calculate_nrmse_elementwise(
    y_true, y_pred, normalization: str = "custom", mask_invalid: bool = True
):
