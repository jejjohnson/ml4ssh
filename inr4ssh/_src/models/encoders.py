...
class CoordinateEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        ), f"Expected input to have {self.in_dim} channels (got {channels} channels)"
...
class GaussianFourierFeatureTransform(CoordinateEncoding):
    def __init__(
        ...
        super().__init__(
            self.get_transform_matrix(in_dim, mapping_size, sigma, seed=seed),
...
def encoder_factory(encoder: str, dim_in: int, config):
    if encoder == "fourier":
        return GaussianFourierFeatureTransform(
            in_dim=dim_in, mapping_size=config.mapping_size, sigma=config.sigma
        )
    elif encoder == "positional":
        return NeRFPositionalEncoding(in_dim=dim_in, n=config.n)
    elif encoder == "identity":
        return IdentityPositionalEncoding(in_dim=dim_in)
    else:
        raise ValueError(f"Unrecognized encoder: {encoder}")
