...
def preprocess_dc20a(cfg) -> None:
    loguru.logger.info("Running preprocess (clean) script...")
    t0 = time.time()

    # CLEANING RAW NADIR FILES
    raw_obs_dir = Path(cfg.raw_obs_dir)

    loguru.logger.info("checking osse files...")
    loguru.logger.debug(f"Checking file: {str(raw_obs_dir)}")
    check_dc20a_files(raw_obs_dir, None, "obs")

    logger.info("Get NADIR track files...")
    alongtrack_files = get_raw_altimetry_files(raw_obs_dir)

    # load them in parallel
    logger.info("Load NADIR tracks (parallel)...")
    ds_nadirs = load_xr_datasets_list(alongtrack_files)

    clean_obs_dir = Path(cfg.get("clean_obs_dir"))
    loguru.logger.debug(f"Checking file: {clean_obs_dir}")
    make_directory(clean_obs_dir)

    for ifilename, ids in ds_nadirs.items():
        # save dataset
        ifilename = Path(clean_obs_dir).joinpath(ifilename)
        logger.info(f"Saving NADIR: {ifilename}...")
        ids.to_netcdf(ifilename)

    # CLEANING RAW SWOT FILES
    logger.info("Get SWOT track files...")
    file_path = get_raw_altimetry_files(raw_obs_dir, "swot")
    ...
    ifilename = Path(clean_obs_dir).joinpath(file_path[0].name)
    logger.info(f"Saving KARIN SWOT: {ifilename}...")
    ds_karin_swot.to_netcdf(ifilename)

    # SWOT NADIR DATA
    logger.info("Get SWOT track files...")
    file_path = get_raw_altimetry_files(raw_obs_dir, "swotnadir")
...
def main(cfg):
    ...
    loguru.logger.info(f"Initializing Preprocessing. Experiment: {experiment}")

    if experiment == "dc20a":
        preprocess_dc20a(cfg.preprocess)
    elif experiment == "dc21a":
        raise NotImplementedError(f"")
    elif experiment == "natl60":
